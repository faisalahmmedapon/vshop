                                        ...
                                        <div class="overflow-x-auto">
                                            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr v-for="brand in brands" :key="brand.id" class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                                        <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white"> {{ brand.name }}</td>
                                                        <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white flex">

                                                            <button class="mr-1" @click="openEditModal(brand)">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                    </svg>

                                                            </button>
                                                            <button class="mr-1" @click="removeBrand(brand)">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
        ...
        >
            ...
            >
                <div class="relative z-0 w-full mb-6 group">
                    <input
                        v-model="useThisFormForBrandCreateOrUpdate.name"
                        type="text"
                        name="name"
                        id="name"
...
const useThisFormForBrandCreateOrUpdate = useForm({
    id: "",
    name: "",
});
...
const createProduct = async () => {
    try {
        ...
        await router.post(
            "/admin/brand/store",
            useThisFormForBrandCreateOrUpdate,
            {
                onSuccess: (page) => {
                    dialogVisible.value = false;
                    useThisFormForBrandCreateOrUpdate.reset();
                    Swal.fire({
                        toast: true,
                        icon: "success",
                        position: "top-end",
...
const openEditModal = (brand) => {
    ...
    useThisFormForBrandCreateOrUpdate.id = brand.id;
    useThisFormForBrandCreateOrUpdate.name = brand.name;
};

const updateProduct = async () => {
    try {
        await router.post(
            "/admin/brand/update/" + useThisFormForBrandCreateOrUpdate.id,
            useThisFormForBrandCreateOrUpdate,
            {
                onSuccess: (page) => {
                    dialogVisible.value = false;
